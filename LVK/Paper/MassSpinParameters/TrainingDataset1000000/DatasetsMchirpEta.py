...
TRAINING_DATASET_FILE = DATA_DIR+'2000000MchripEtaSpinTrainingDataset.csv'
VALIDATION_DATASET_FILE = DATA_DIR+'200000MchripEtaSpinValidationDataset.csv'
TEST_DATASET_FILE = DATA_DIR+'10000MchripEtaSpinTestDataset.csv'
...
def dataset_generation(size_of_dataset, output_of_dataset):
    ...
    for ref_mchirp, ref_eta, mchirp, eta, ref_s1, ref_s2, s1, s2 in zip(ref_mchirp, ref_eta, mchirp, eta, reference_spin[:, 0], reference_spin[:, 1], spin[:, 0], spin[:, 1]):
        ...
        parameters_list.append([ref_mchirp, ref_eta, ref_s1, ref_s2, mchirp, eta, s1, s2, match])
    ...
    MassSpinMatchDataset =  pd.DataFrame(data=(parameters_list), columns=['ref_mchirp', 'ref_eta', 'ref_spin1', 'ref_spin2', 'mchirp', 'eta', 'spin1', 'spin2', 'match'])
