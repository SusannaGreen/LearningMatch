...
TRAINING_DATASET_FILE_PATH = DATA_DIR+r'2000000MchirpEtaSpinTrainingDataset.csv'
VALIDATION_DATASET_FILE_PATH = DATA_DIR+r'200000MchirpEtaSpinValidationDataset.csv'

#Define ouput location of the Standard.Scaler()
STANDARD_SCALER = DATA_DIR+'2000000MchirpEtatandardScaler.bin'

#Define output location of the LearningMatch model
LEARNINGMATCH_MODEL = DATA_DIR+'2000000MchirpEtaLearningMatchModel.pth'

#Define output location for the training and validation loss
LOSS = DATA_DIR+'2000000MchirpEtaTrainingValidationLoss.csv'
...
TrainingBank[['ref_mchirp', 'ref_eta', 'mchirp', 'eta']] = scaler.fit_transform(TrainingBank[['ref_mchirp', 'ref_eta', 'mchirp', 'eta']])
ValidationBank[['ref_mchirp', 'ref_eta', 'mchirp', 'eta']] = scaler.transform(ValidationBank[['ref_mchirp', 'ref_eta', 'mchirp', 'eta']])

#Splitting into input (i.e. the template parameters) and output (the match)
x_train = np.vstack((TrainingBank.ref_mchirp.values, TrainingBank.ref_eta.values, 
                     TrainingBank.ref_spin1.values, TrainingBank.ref_spin2.values, 
                     TrainingBank.mchirp.values, TrainingBank.eta.values,
                     TrainingBank.spin1.values, TrainingBank.spin2.values)).T
y_train = TrainingBank.match.values

x_val = np.vstack((ValidationBank.ref_mchirp.values, ValidationBank.ref_eta.values,
                   ValidationBank.ref_spin1.values, ValidationBank.ref_spin2.values, 
                   ValidationBank.mchirp.values, ValidationBank.eta.values,
