...
error_list = []
for x, y in zip(error, to_cpu_np(x_test)):
    if abs(x) > 0.2:
...
#np.savetxt("NEWMassSpinPlot.csv", loss_array, delimiter=",")
#np.savetxt("NEWMassSpinPlot.csv", val_loss_array, delimiter=",")
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('NEWMassSpinLossCurve.pdf')
...
plt.savefig('NEWMassSpinActualPredicted.pdf', dpi=300)
...
plt.savefig('NEWMassSpinError.pdf', dpi=300)
...
plt.savefig('NEWMassSpinActualPredicted2.pdf', dpi=300)