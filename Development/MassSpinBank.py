...
SIZE = 1000000 #size of the desired template bank 
LOW_FREQ = 12
...
for ref_m1, ref_m2, m1, m2, ref_s1, ref_s2, s1, s2 in zip(reference_mass[:, 0], reference_mass[:, 1], mass[:, 0], mass[:, 1], reference_spin[:, 0], reference_spin[:, 1], spin[:, 0], spin[:, 1]):
    ...
    match, Index = template.match(template_reference, psd=psd, low_frequency_cutoff=15) # This was 15 and everything else was 12??? I thought it was a bug
...
MassSpinMatchDataset.to_csv('1000000MassSpinMatchDataset2.csv', index = False)

print("The size of the dataset", SIZE)
print("This generated a dataset called: 1000000MassSpinMatchDataset2")
